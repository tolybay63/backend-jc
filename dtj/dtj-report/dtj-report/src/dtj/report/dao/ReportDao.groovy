...
class ReportDao extends BaseMdbUtils {
    ...
    @DaoMethod
    List<Map<String, Object>> loadReportFilterOptions(Map<String, Object> params) {
//        Map<String, Long> map = apiMeta().get(ApiMeta).getIdFromCodOfEntity("Cls", "Cls_ReportPage", "")
        ...
        //
        if (UtCnv.toLong(params.get("pageId")) == 0)
            throw new XError("[pageId] не указан")
        //mdb.outTable(st)
        List<Map<String, Object>> listSt = loadReportPage(UtCnv.toLong(params.get("pageId")))
        //
        if (listSt.size() == 0)
            throw new XError("[pageId] не найден")
        //
        List<Map<String, Object>> listComplex = (List<Map<String, Object>>) listSt.get(0).get("complex")
        if (UtCnv.toLong(params.get("containerId")) > 0) {
            listComplex.forEach {Map<String, Object> it -> {
                if (it.get("objLinkToView") == params.get("containerId")) {
                    Store stTmp = loadReportPresentation(UtCnv.toLong(it.get("objLinkToView")))
                    if (stTmp.size() > 0 && UtCnv.toLong(stTmp.get(0).get("parent")) > 0) {
                        List<Map<String, Object>> listConfig =loadReportConfiguration(UtCnv.toLong(stTmp.get(0).get("parent")))
                    }
                    mdb.outTable(stTmp)
                }
            }}
            ...
            listComplex.forEach {Map<String, Object> it -> {
                ...
                if (stTmp.size() > 0 && UtCnv.toLong(stTmp.get(0).get("parent")) > 0) {
                    List<Map<String, Object>> listConfig = loadReportConfiguration(UtCnv.toLong(stTmp.get(0).get("parent")))
                    mapRes.put("filterVal", listConfig.get(0).get("FilterVal"))
                    //
                    Store stSource = loadReportSource(UtCnv.toLong(listConfig.get(0).get("parent")))
                    mapRes.put("URL", stSource.get(0).get("URL"))
                    mapRes.put("MethodBody", stSource.get(0).get("MethodBody"))
                    //
                    mdb.outTable(stSource)
                }
                lstRes.add(mapRes)
                mdb.outTable(stTmp)
            }}
    ...
    Store reportTaskLog(Map<String, Object> params) {
        ...
        // Получение записей из Журнала задач
        Store st = loadSqlService("""
            select o.id,
                ...
                null as plan_StartLink, null as plan_FinishLink, null as plan_PlanDateEnd, null as plan_FactDateEnd,
                ...
                null as RegistrationDateTime, null as AssignDateTime, null as objInspection,
                null as par_CreationDateTime, null as def_CreationDateTime,
            ...
            where ${whe}
        """, "Report.ReportTaskLog", "repairdata")
        //... Пересечение
        ...
        Store stWP = loadSqlService("""
            select o.id,
                ...
                v10.dateTimeVal as StartLink,
                v11.dateTimeVal as FinishLink,
            ...
            from Obj o
                ...
                left join DataPropVal v9 on d9.id=v9.dataProp
                left join DataProp d10 on d10.objorrelobj=o.id and d10.prop=${map.get("Prop_StartLink")}
                left join DataPropVal v10 on d10.id=v10.dataProp
                left join DataProp d11 on d11.objorrelobj=o.id and d11.prop=${map.get("Prop_FinishLink")}
        ...
        for (StoreRecord r in st) {
            ...
            if (rWP != null) {
                ...
                r.set("plan_StartLink", rWP.getLong("StartLink"))
                r.set("plan_FinishLink", rWP.getLong("FinishLink"))
        ...
        Store stFault = loadSqlService("""
            select o.id,
                v1.obj as objInspection,
                v2.dateTimeVal as CreationDateTime,
                v3.obj as objDefect, null as nameDefect, null as objComponent, null as nameComponent
            from Obj o
                left join DataProp d1 on d1.objorrelobj=o.id and d1.prop=${map.get("Prop_Inspection")}
                left join DataPropVal v1 on d1.id=v1.dataprop
                left join DataProp d2 on d2.objorrelobj=o.id and d2.prop=${map.get("Prop_CreationDateTime")}
                left join DataPropVal v2 on d2.id=v2.dataprop
                left join DataProp d3 on d3.objorrelobj=o.id and d3.prop=${map.get("Prop_Defect")}
                left join DataPropVal v3 on d3.id=v3.dataprop
        ...
        stWP = loadSqlService("""
            ...
            from Obj o
                ...
                left join DataPropVal v3 on d3.id=v3.dataProp
                left join DataProp d4 on d4.objorrelobj=o.id and d4.prop=${map.get("Prop_Incident")}
                left join DataPropVal v4 on d4.id=v4.dataProp          
            where o.id in (0${idsWorkPlan.join(",")})
        """, "", "plandata")
        //
    ...
    @DaoMethod
    Store reportTaskLog_(Map<String, Object> params) {
        Map<String, Long> map = apiMeta().get(ApiMeta).getIdFromCodOfEntity("RelTyp", "RT_ParamsComponent", "")
        long reltypParamsComponent = map.get("RT_ParamsComponent")
        map = apiMeta().get(ApiMeta).getIdFromCodOfEntity("Cls", "Cls_TaskLog", "")
        ...
        Store st = loadSqlService("""
            select o.id,
                ...
                null as plan_StartKm, null as plan_FinishKm, null as plan_StartPicket, null as plan_FinishPicket,
                null as plan_StartLink, null as plan_FinishLink, null as plan_PlanDateEnd, null as plan_FactDateEnd,
                null as plan_objSection, null as plan_nameSection, null as plan_objIncident,
                null as objEvent, null as nameEvent, null as pvStatus, null as fvStatus, null as nameStatus,
                null as pvCriticality, null as fvCriticality, null as nameCriticality,
                null as RegistrationDateTime, null as AssignDateTime,
                null as objComponent, null as nameComponent, null as objInspection,
                null as objParameterLog, null as par_CreationDateTime,
                null as relobjComponentParams, null as nameComponentParams,
                null as ParamsLimit, null as ParamsLimitMax, null as ParamsLimitMin,
                null as objFault, null as def_CreationDateTime, null as objDefect, null as nameDefect,
                null as ins_objWorkPlan, null as ins_FactDateEnd,
                null as ins_plan_objWork, null as ins_plan_fullNameWork,
                null as ins_plan_PlanDateEnd, null as ins_plan_FactDateEnd, 
                null as ins_plan_objIncident
